...
phi <- (1+sqrt(5))/2
#' Define paths:
filename <- "sts_preprocessing_neg"
...
MZML_PATH <- "C:/Users/User/Documents/sts_mzML_neg"
MZML_PATH <- "/data/massspec/mzML/2017/2017_08/"
...
The following code allows preprocessing of data registered in negative ion mode.
...
pd <- read.table("data/mitra-short-term-files.txt", sep = "\t",
...
```{r neg-data_neg, message = FALSE, eval = !file.exists(paste0(RDATA_PATH, "data_neg_read_raws.RData"))}
#' Read the data
...
save(data_neg, file = paste0(RDATA_PATH, "data_neg_read_raws.RData"))
```

```{r, echo = FALSE}
load(paste0(RDATA_PATH, "data_neg_read_raws.RData"))
...
tab <- pData(data_neg)[, c("mzML_file", "storage", "time")]
...
```{r neg-calc-tic, message = FALSE, eval = !file.exists(paste0(RDATA_PATH, "chrs_bpc_raw.RData"))}
...
The base peak chromatograms look quite similar across the different storage
methods. Like seen in the data that was generated in positive mode, the largest
differences can be observed at a retention time range from 110 to 150 seconds.
...
```{r neg-bin-chr, echo = FALSE, message = FALSE, warning = FALSE, fig.path = IMAGE_PATH, fig.width = 12, fig.height = 12, fig.cap = "Samples clustered by base peak chromatogram. Clustering bases on complete linkage hierarchical clustering of pairwise correlations of log2 base peak signals."}
...
ann <- pData(data_neg)[, c("storage", "time")]
res <- pheatmap(cormat, annotation_col = ann,
                annotation_color = list(storage = col_storage))
```

The base peak chromatograms does not cluster by storage method, but there is a
small clustering effect due to the storage duration can be observed, the samples
...
Overall seen, the signal is comparable between the samples, but shows a higher
variance in intensity compared to the data generated in negative mode.
...
```{r neg-bpc-pca-injection-idx, fig.path = IMAGE_PATH, fig.width = 10, fig.height = 10, fig.cap = "PCA of the base peak signal. Numbers indicate the injection index."}
pc <- prcomp(t(log2(do.call(cbind, lapply(chrs_bin, intensity)))),
             scale = FALSE, center = TRUE)
par(mfrow = c(2, 2))
plot_pca(pc, col = col_storage[as.character(data_neg$storage)],
...
Looking at the PC1 plot, we see that most of the QC samples are clustered
together, whereas it is hard to differentiate between the different storage
methods.

We then redo the PCA analysis but colour the samples according to storage
duration (time):

```{r neg-bpc-pca-injection-idx-time, fig.path = IMAGE_PATH, fig.width = 10, fig.height = 10, fig.cap = "PCA of the base peak signal. Numbers indicate the injection index."}
pc <- prcomp(t(log2(do.call(cbind, lapply(chrs_bin, intensity)))),
             scale = FALSE, center = TRUE)
par(mfrow = c(2, 2))
plot_pca(pc, col = col_time[as.character(data_neg$time)],
         pc_x = 1, pc_y = 2, labels = data_neg$inj_idx)
...
2 days are also mainly clustered together.
...
*std_info_neg.txt* file, which is then loaded and it is used to continue
the study.
...
```{r neg-peaks-summary-plot, warning = FALSE, message = FALSE, fig.cap = "Summaries of identified chromatographic peaks. Shown are the number of detected peaks, their m/z widths and their integrated signal in each measured sample.", fig.width = 12, fig.height = 10, fig.path = IMAGE_PATH}
...
with a high peak density (see plot below). We also set `minFraction` to 0.95 to
exclude those peaks that are found in less than 95% of samples and `bw` to 2 as
the standard deviation of the smoothing kernel.

Then, we specify the subset that we want to perform alignment on, the `"QC"`
...
```{r neg-mz-distance-peaks-plot, message = FALSE, warning = FALSE, fig.cap = "Position of detected chromatographic peaks within a sample. Shown are chromatographic peaks in a region with high peak density. The horizontal blue line represents the bin width if binSize = 0.02 was used.", fig.path = IMAGE_PATH}
...
```{r neg-correspondence-kc-plots, echo = FALSE, warning = FALSE, eval = plot_eics, message = FALSE}
std_info_chr <- chromatogram(
    data_neg, mz = std_info_mz_rt_obs[, c("mzmin", "mzmax")],
    rt = rtr, aggregationFun = "max")
...
in more than 90 % of samples is low (about 0,0003 %).
