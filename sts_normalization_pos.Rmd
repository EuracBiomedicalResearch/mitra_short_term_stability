...
colData(res_pos)$time_stamp <- as.character(colData(res_pos)$time_stamp)
```
...
res_pos$storage_time <- factor(
    res_pos$storage_time, levels = c("RT_2h", "RT_6h", "RT_1d", "RT_2d",
...
cond_ints <- lapply(levels(res_pos$storage_time), function(z) {
    rowMeans(log2(assay(res_pos, "raw_filled")[, res_pos$storage_time == z]),
             na.rm = TRUE)
})
names(cond_ints) <- levels(res_pos$storage_time)
...
is_data <- filterMatches(is_data,
                         queryValue = c("FT00394", "FT00394", "FT00395"),
