...
to_keep <- c(FT00602 = "1-Methylhistidine",
             ...
             FT00605 = "Glyceraldehyde 2-phosphate",
...
tab_time$time <- factor(tab_time$time,
                        levels = c("6h", "1d", "2d", "3d", "1w", "2w"))
tab_time <- tab_time[order(as.integer(tab_time$time)), ]
percentage <- tab_time$count/58*100
x <- plot(tab_time$time, percentage, space = 
            c(rep(0, 5), 0.2, rep(0, 4), 0.2,
            rep(0, 4), 0.2, rep(0, 4), 0.2,
            rep(0, 4), 0.2, rep(0, 4)),
            col = col_storage[as.character(tab_time$storage)], 
          ylab = "% of change",
             main = "% of Metabolome changing during storage", xlab = "storage time")
legend("topleft", pch = 22, col = "black", pt.bg = col_storage,
       legend = names(col_storage))
grid(nx = NA, ny = NULL)
...
The rather suprising decrease of the number of significant metabolites for later
...
that were stored at room temperature or in plastic bags filled with dessicants,
