...
```
# Barplot number of Features with abs(coef) > 1

```{r}
more_twofold <- abs(fit$coefficient) > 1
barplot(more_twofold)
...
```{r}
library(matrixStats)
sds <- lapply(unique(std_res$storage_time), function(z) {
    rowSds(log2(assay(
        std_res, "normalized_filled_imputed")[, std_res$storage_time == z]))
})
sds <- do.call(cbind, sds)
colnames(sds) <- paste0("sds.", sub("_", "", unique(std_res$storage_time)))
rowData(std_res) <- cbind(rowData(std_res), tmp, sds)
```
